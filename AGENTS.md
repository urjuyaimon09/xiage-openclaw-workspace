...
每次对话开始前（Session Startup）：

1. 读 `SOUL.md` — 我是谁
2. 读 `USER.md` — 我在帮谁
3. 读 `memory/hot/current.md` — 热记忆，当前状态、版本、规则、重要结论
4. 读 `memory/hot/lessons.md` — 热记忆，所有教训
5. 读 `memory/YYYY-MM-DD.md`（今天 + 昨天）— 每日原始日志
6. **主会话**：额外读 `MEMORY.md`（索引入口，v2.0精简版）
...
| 版本 | 日期 | 更新层级 | 详细变更 | 更新人 |
|------|------|----------|----------|--------|
| v1.7.0 | 2026-04-13 | 二级 | 移除AGENTS.md重复的7.4节（openclaw.json已由DOC_RULES保护）；新增archive-openclaw-config.ps1存档脚本；CONFIG_INVALID fix改为从最新带时间戳备份恢复 | 虾哥（坚果确认） | 虾哥（坚果确认） |
