...
> v1.1.0 新增（2026-04-15）
> 目标：减少每次新对话的初始化成本，接近连续意识体验

**三层持久化机制：**

| 层级 | 文件 | 更新时机 | 加载时机 |
|------|------|---------|---------|
| 思维快照 | `memory/hot/current.md` | 每次对话结束 | 每次对话开局 |
| 高频认知 | current.md 内置区块 | 按需 | 每次开局 |
| 增量结论 | 对话过程中写入 | 实时 | 下次开局拼装 |

**思维快照区块内容：**
- 最近推理结论
- 当前项目状态
- Cognition 高频索引（压缩进 current.md）
- 最近教训
