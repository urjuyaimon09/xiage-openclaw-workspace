...
function 采集被认可度() {
  ...
  if (fs.existsSync(认可记录文件)) {
    try {
      ...
      const 认可 = 记录.positive || 0;
      const 否定 = 记录.negative || 0;
      ...
      if (总数 === 0) {
        ...
        // 认可度 = (认可 - 否定*0.5) / 总数，标准化到[0,1]
        认可度 = Math.max(0, Math.min(1, (认可 - 否定 * 0.5) / 总数));
      }
    } catch (e) {
      认可度 = 0.5;
    }
...
function 采集进化速度() {
  ...
  if (fs.existsSync(进化记录文件)) {
    try {
      ...
      const 能力提升 = 记录.capabilityGain || 0;
      const 协作改善 = 记录.collabImprovement || 0;
      
      // 综合评分
      进化速度 = Math.min(1, (新增认知 * 0.1 + 能力提升 * 0.5 + 协作改善 * 0.4));
    } catch (e) {
      进化速度 = 0.5;
    }
  }
